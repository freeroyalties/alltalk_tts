...
def get_medium_size_wav(folder, min_size_kb=500, max_size_kb=2000):
    wav_files = [file for file in Path(folder).glob("*.wav")]
    medium_files = [
        file
        for file in wav_files
        if min_size_kb * 1024 <= file.stat().st_size <= max_size_kb * 1024
    ]
    if not medium_files:
        raise FileNotFoundError(f"No medium-sized WAV files found in {folder}")
    return str(medium_files[0])
...
def create_test_sentences(sentences, speaker_reference, language):
    return [
        {
            "text": sentence,
...
def train_gpt(
    ...
    audio_folder = "./finetune/put-voice-samples-in-here"
    SPEAKER_REFERENCE = [get_medium_size_wav(audio_folder)]
    ...
    config = GPTTrainerConfig(
        ...
        test_sentences=create_test_sentences(sentences, SPEAKER_REFERENCE, LANGUAGE),
...
def run_tts(lang, tts_text, speaker_audio_file, speed):
...
if __name__ == "__main__":
    ...
    with gr.Blocks(theme=theme, css=custom_css) as demo:
        ...
        with gr.Tab("🔜 What to do next"):
            ...
            tts_btn.click(
                fn=run_tts,
                inputs=[tts_language, tts_text, speaker_reference_audio, speed],
