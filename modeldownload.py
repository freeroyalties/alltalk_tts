...
with open(
    this_dir / "system" / "requirements" / "requirements_standalone.txt", "r"
) as req_file:
...
def check_tts_version():
    try:
        ...
        print(
            f"[{params['branding']}Startup] \033[92mCurrent TTS Version    : \033[93m"
            + tts_version
            + "\033[0m"
        )
        ...
        if version.parse(tts_version) < version.parse(tts_version_required):
            ...
            print(
                f"[{params['branding']}Startup] \033[92mCurrent TTS Version is :\033[93m Up to date\033[0m"
            )
...
def check_torch_version():
    ...
    print(
        f"[{params['branding']}Startup] \033[92mCurrent Python Version :\033[93m {python_version}\033[0m"
    )
    print(
        f"[{params['branding']}Startup] \033[92mCurrent PyTorch Version:\033[93m {pytorch_version}\033[0m"
    )
    if cuda_version is None:
        print(
            f"[{params['branding']}Startup] \033[92mCurrent CUDA Version   :\033[91m CUDA is not available\033[0m"
        )
    else:
        print(
            f"[{params['branding']}Startup] \033[92mCurrent CUDA Version   :\033[93m {cuda_version}\033[0m"
        )
...
def fetch_latest_commit_sha_and_date(owner, repo):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"[{params['branding']}Startup] \033[92m{params['branding']}Github updated :\033[91m Failed to fetch the latest commits due to an unexpected response from GitHub"
            )
            return None, None
    except ConnectionError:
        # This block is executed when a connection error occurs
        print(
            f"[{params['branding']}Startup] \033[92m{params['branding']}Github updated :\033[91m Could not reach GitHub to check for updates\033[0m"
        )
        return None, None
...
print(
    f"[{params['branding']}Startup] \033[92mModel is available     :\033[93m Checking\033[0m"
)
for filename, url in files_to_download.items():
    ...
    if not destination.exists():
        print(
            f"[{params['branding']}Startup] \033[92mModel Downloading file : \033[93m"
            + filename
            + "\033[0m"
        )
        download_file(url, destination)
print(
    f"[{params['branding']}Startup] \033[92mModel is available     :\033[93m Checked\033[0m"
)
...
if latest_commit_sha and latest_commit_sha != last_known_commit_sha:
    ...
    print(
        f"[{params['branding']}Startup] \033[92m{params['branding']}Github updated :\033[93m {formatted_date}\033[0m"
    )
